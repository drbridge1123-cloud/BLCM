...
/* PSM content styles */
.psm-subtitle { font-size: 12px; font-weight: 500; color: var(--gold, #C9A84C); margin-top: 2px; }
...
<div x-show="showRequestModal" class="sp-modal-overlay" style="display:none;" @keydown.escape.window="showRequestModal = false" @click="showRequestModal = false">
    <div class="sp-modal-box" style="max-width:512px" @click.stop>
        <div class="sp-modal-header">
            <div><div class="sp-modal-title">New Request</div><div class="mrt-subtitle" x-text="reqForm._carrierLabel"></div></div>
            <button type="button" class="sp-modal-close" @click="showRequestModal = false"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
        <div class="sp-modal-body">
            <div class="mrt-row">
                ...
                <div><label class="sp-form-label">Method <span class="mrt-req">*</span></label>
                    <select x-model="reqForm.request_method" @change="updateRecipient()" class="sp-select"><option value="">Select...</option><option value="email">Email</option><option value="fax">Fax</option><option value="portal">Portal</option><option value="phone">Phone</option><option value="mail">Mail</option></select>
                </div>
            </div>
            <div class="mrt-row">
            ...
            <div><label class="sp-form-label">Template</label><select x-model="reqForm.template_id" class="sp-select"><template x-for="t in hlTemplates" :key="t.id"><option :value="t.id" x-text="t.name + (t.is_default ? ' (Default)' : '')"></option></template></select></div>
            ...
            <div x-show="reqForm.request_method === 'email' || reqForm.request_method === 'fax'"
                ...
                <button type="button" @click="showDocs = !showDocs" class="w-full flex items-center justify-between py-2 text-left">
                    <div class="flex items-center gap-2"><label class="sp-form-label" style="margin:0;cursor:pointer;">Attachments</label><span x-show="selectedIds.length > 0" class="text-[10px] font-bold bg-gold/20 text-gold px-1.5 py-0.5 rounded-full" x-text="selectedIds.length + ' selected'"></span></div>
                ...
                <div x-show="showDocs" x-collapse>
                    ...
                    <div class="mt-2 text-[10px] text-v2-text-light">For PDF field overlay, use <a :href="'/blcm/frontend/pages/bl-cases/detail.php?id=' + reqForm._caseId" class="text-gold underline" target="_blank">Case Detail</a> &rarr; Documents</div>
                </div>
            </div>
        </div>
...
<!-- Log Receipt Modal -->
<div x-show="showReceiptModal" class="sp-modal-overlay" style="display:none;" @keydown.escape.window="showReceiptModal = false" @click="showReceiptModal = false">
    <div class="sp-modal-box" style="max-width:512px" @click.stop>
        <div class="sp-modal-header"><div><div class="sp-modal-title">Log Receipt</div><div class="mrt-subtitle" x-text="receiptForm._label"></div></div><button type="button" class="sp-modal-close" @click="showReceiptModal = false"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>
        <div class="sp-modal-body">
            <div class="mrt-row">
                <div><label class="sp-form-label">Received Date <span class="mrt-req">*</span></label><input type="date" x-model="receiptForm.received_date" class="sp-search"></div>
                <div><label class="sp-form-label">Received Via <span class="mrt-req">*</span></label><select x-model="receiptForm.received_method" class="sp-select"><option value="">Select...</option><option value="email">Email</option><option value="fax">Fax</option><option value="portal">Portal</option><option value="mail">Mail</option><option value="other">Other</option></select></div>
            </div>
            <div class="mb-4"><label class="sp-form-label mb-2">Records Received</label>
            ...
            <template x-if="!receiptForm.is_complete"><div class="mb-4"><label class="sp-form-label">What's still missing?</label><input type="text" x-model="receiptForm.incomplete_reason" class="sp-search" placeholder="e.g., Still waiting for billing records"></div></template>
            <div><label class="sp-form-label">Notes</label><textarea x-model="receiptForm.notes" rows="2" class="sp-search" style="resize:vertical;min-height:70px;line-height:1.5" placeholder="Optional notes..."></textarea></div>
        </div>
        <div class="sp-modal-footer">
            <button @click="showReceiptModal = false" class="sp-btn">Cancel</button>
            <button @click="submitReceipt()" :disabled="saving" class="sp-new-btn" style="background:#059669;"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span x-text="saving ? 'Saving...' : 'Log Receipt'"></span></button>
        </div>
    </div>
</div>

<!-- Bulk Request Modal -->
<div x-show="showBulkRequestModal" class="sp-modal-overlay" style="display:none;" @keydown.escape.window="closeBulkRequestModal()" @click="closeBulkRequestModal()">
    <div class="sp-modal-box" style="max-width:896px" @click.stop>
        <div class="sp-modal-header"><div><div class="sp-modal-title">Bulk Follow-Up Request</div></div><button type="button" class="sp-modal-close" @click="closeBulkRequestModal()"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>
        <div class="sp-modal-body" style="overflow-y:auto;">
            ...
            <div class="mb-6 grid grid-cols-2 gap-4">
                <div><label class="sp-form-label">Request Date</label><input type="date" x-model="bulkRequestForm.request_date" class="sp-search"></div>
                <div><label class="sp-form-label">Request Method</label><select x-model="bulkRequestForm.request_method" class="sp-select"><option value="email">Email</option><option value="fax">Fax</option><option value="portal">Portal</option><option value="phone">Phone</option><option value="mail">Mail</option></select></div>
                <div><label class="sp-form-label">Follow-up Date</label><input type="date" x-model="bulkRequestForm.followup_date" class="sp-search"></div>
                <div><label class="sp-form-label">Request Type</label><select x-model="bulkRequestForm.request_type" class="sp-select"><option value="follow_up">Follow-Up</option><option value="re_request">Re-Request</option><option value="initial">Initial</option></select></div>
                <div class="col-span-2" x-show="bulkTemplates.length > 0"><label class="sp-form-label">Letter Template</label><select x-model="bulkRequestForm.template_id" class="sp-select"><option value="">Default (built-in)</option><template x-for="t in bulkTemplates" :key="t.id"><option :value="t.id" x-text="t.name + (t.is_default ? ' (Default)' : '')"></option></template></select></div>
            </div>
            <div class="mb-6"><label class="sp-form-label">Notes (optional)</label><textarea x-model="bulkRequestForm.notes" rows="2" class="sp-search" style="resize:vertical;min-height:70px;line-height:1.5" placeholder="Additional notes..."></textarea></div>
            <div class="mb-6"><h3 class="text-sm font-semibold text-v2-text mb-3">Cases & Recipients</h3>
                <div class="border border-v2-card-border rounded-lg overflow-hidden"><table class="w-full text-sm"><thead class="bg-v2-bg"><tr><th class="px-3 py-2 text-left">Case #</th><th class="px-3 py-2 text-left">Client</th><th class="px-3 py-2 text-left">Recipient</th><th class="px-3 py-2 w-20">Action</th></tr></thead><tbody>
                    <template x-for="(caseItem, index) in bulkRequestCases" :key="caseItem.id"><tr class="border-t border-v2-bg"><td class="px-3 py-2 font-medium text-gold" x-text="caseItem.case_number"></td><td class="px-3 py-2" x-text="caseItem.client_name"></td><td class="px-3 py-2"><input type="text" x-model="caseItem.recipient" class="sp-search" placeholder="Auto-detect"></td><td class="px-3 py-2 text-center"><button @click="removeFromBulk(index)" class="text-red-600 hover:text-red-700"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></td></tr></template>
                </tbody></table></div>
            </div>
        </div>
        <div class="sp-modal-footer" style="justify-content:space-between;">
            <button @click="closeBulkRequestModal()" class="sp-btn">Cancel</button>
            <div class="flex gap-3">
                <button @click="previewBulkRequests()" class="sp-btn" style="border-color:var(--gold);color:var(--gold);">Preview All</button>
                <button @click="createAndSendBulkRequests()" :disabled="bulkRequestCases.length === 0" class="sp-new-btn"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>Create & Send</button>
            </div>
        </div>
    </div>
</div>
...
<!-- Bulk Preview Modal -->
<div x-show="showBulkPreviewModal" class="sp-modal-overlay" style="display:none;" @keydown.escape.window="closeBulkPreviewModal()" @click="closeBulkPreviewModal()">
    <div class="sp-modal-box" style="max-width:1024px" @click.stop>
        <div class="sp-modal-header"><div><div class="sp-modal-title">Preview Bulk Requests</div><div class="mrt-subtitle">Combined letter for <span x-text="bulkPreviewCaseCount"></span> case(s) to <span x-text="bulkPreviewProviderName"></span></div></div><button type="button" class="sp-modal-close" @click="closeBulkPreviewModal()"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>
        <div class="sp-modal-body" style="flex:1;overflow-y:auto;"><div x-html="bulkPreviewHtml"></div></div>
        <div class="sp-modal-footer" style="justify-content:space-between;"><button @click="closeBulkPreviewModal()" class="sp-btn">Close</button><button @click="confirmAndSendBulk()" class="mrt-btn-send"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>Send All</button></div>
    </div>
</div>

<!-- Preview & Send Modal -->
<div x-show="showPreviewModal" class="sp-modal-overlay" style="display:none;" @keydown.escape.window="showPreviewModal && closePreviewModal()" @click="closePreviewModal()">
    <div class="sp-modal-box sp-modal-box-lg" @click.stop>
        <div class="sp-modal-header">
            ...
            <div class="psm-header-actions">
                ...
                <button class="sp-modal-close" @click="closePreviewModal()"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
        </div>
        <div class="psm-toolbar"><div style="display:flex;gap:12px;"><div style="flex:1;"><label class="sp-form-label" x-text="previewData.method === 'email' ? 'Recipient Email' : 'Recipient Fax Number'"></label><input type="text" x-model="previewData.recipient" class="sp-search" :placeholder="previewData.method === 'email' ? 'provider@example.com' : '(212) 555-1234'"></div><div style="flex:1;" x-show="previewData.method === 'email'"><label class="sp-form-label">Subject</label><input type="text" x-model="previewData.subject" :readonly="!isEditingLetter" class="sp-search"></div></div></div>
        ...
        <div class="sp-modal-footer" style="justify-content:space-between;align-items:center">
            ...
            <div style="display:flex;gap:10px;">
                <button @click="closePreviewModal()" class="sp-btn">Cancel</button>
